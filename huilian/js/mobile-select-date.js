...
(function(root, factory) {
	...
	MobileSelectDate.prototype = {
		init: function(settings) {
			...
			this.separator = "/";
			var now = new Date();
			this.settings.value = this.settings.value || $(this.settings.trigger).text() || now.getFullYear() + "/" + ("0" + (now.getMonth() + 1)).slice(-2) + '/' + ("0" + (now.getDate())).slice(-2);
			...
			this.text = this.settings.text || this.trigger.text().split(' ') || ['', '', ''];
